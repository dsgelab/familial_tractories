...
for i in tqdm.tqdm(range(len(eps))):
    ...
    df = df.merge(df_events_sub[['FINNGENID', 'YEAR']], 'left', left_on='FINREGISTRYID', right_on='FINNGENID')
    df['ch_ep'+str(i)] = df.YEAR.fillna(2022.)
    df = df.drop(columns=['FINNGENID', 'YEAR'])
    df = df.merge(df_events_sub[['FINNGENID', 'YEAR']], 'left', left_on='id_mother', right_on='FINNGENID')
    df['mo_ep'+str(i)] = df.YEAR.fillna(2022.)
    df = df.drop(columns=['FINNGENID', 'YEAR'])
    df = df.merge(df_events_sub[['FINNGENID', 'YEAR']], 'left', left_on='id_father', right_on='FINNGENID')
...
df = df.rename(columns={'outcome': 'ch_ep1'})
df = df.rename(columns={'ch_ep2': 'outcome'})
plot_odds_ratio(stat, eps, demos)
